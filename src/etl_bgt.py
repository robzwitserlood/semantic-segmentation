...
def data_already_stored(config):
...
def ready_to_start(overwrite, config):
    ...
    :rtype: bool
    """
    if data_already_stored(config) and not overwrite:
        return False
    return True


def get_params(geo_filter, config):
...
def get_download_request_id(params, config):
...
def get_download_link(download_id, config):
...
def download_extract_and_store(download_link, config):
...
def concat_data_from_feature_type(feature_type, gdf_in, config):
...
def drop_unused_rows(gdf):
    ...
    gdf = gdf[mask1]
    mask2 = gdf.label != 'unknown'
    gdf = gdf[mask2]
    return gdf


def transform(crs, config):
    """Transform bgt data.
    ...
    for feature_type in config['bgt']['api_params']['featuretypes']:
        gdf = concat_data_from_feature_type(feature_type, gdf, config)
...
def load(config):
    ...
    return gpd.read_parquet(file_dir)


def process(geo_filter, crs, config, overwrite=False):
    ...
    :param crs: coordinate reference frame of the geometries
    :type crs: pyproj.crs.crs.CRS
    :param config: configuration dict (build from src/config.py via config_etl_bgt)
    :type config: dict
    :param overwrite: indicator if stored bgt data should be overwritten
    :type overwrite: bool
    ...
    ...
    :rtype: geopandas.geodataframe.GeoDataFrame
    """
    if ready_to_start(overwrite, config):
        params = get_params(geo_filter, config)
        download_id = get_download_request_id(params, config)
        download_link = get_download_link(download_id, config)
        download_extract_and_store(download_link, config)
        transform(crs, config)
    return load(config)
